...
export function ArrangeStudentsDrawer({
  ...
  const distribution = useMemo((): RangeDistribution[] => {
    return scoreRanges.map((scoreRange) => {
      ...
      const rangeStudents = students.filter((s) => {
        const isInRange = scoreRange.max === 10
  ...
  const handleRangeChange = (id: string, field: 'min' | 'max', value: string) => {
    ...
    setScoreRanges((prev) =>
      prev.map((range) => (range.id === id ? { ...range, [field]: numValue } : range))
    )
  }
        ...
        <div className='flex-1 overflow-y-auto min-h-0'>
          ...
          {!showPreview && (
            <div className='space-y-4'>
              <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
                <div className='text-sm font-medium text-greyscale-900'>
                  Chọn lớp học cho từng khoảng điểm
                </div>
                <Button
                  type='button'
                  variant='outline'
          ...
          {showPreview && (
            <div className='space-y-4'>
              ...
              {/* Enrolled Students */}
              {distribution.filter((d) => d.classId).map((dist) => (
                  ...
                  <div className='space-y-1.5'>
                    ...
                    {failedStudents.length > 5 && (
                      <div className='text-xs text-red-600'>
                        ...và {failedStudents.length - 5} học sinh khác
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className='mx-auto w-full max-w-4xl flex flex-col-reverse sm:flex-row gap-2'>
            {!showPreview ? (
              ...
              <>
                ...
                <Button onClick={handleSubmit} disabled={arrangeStudentsMutation.isPending} className='w-full sm:flex-1'>
