...
class Logger {
   ...
   */
  formSubmission(formName: string, success: boolean, data?: unknown, context?: LogContext): void {
    const message = `Form Submission: ${formName} ${success ? 'succeeded' : 'failed'}`
   ...
   */
  auth(action: string, success: boolean, data?: unknown, context?: LogContext): void {
...
export const log = {
  ...
  user: (action: string, data?: unknown, context?: LogContext) => logger.userAction(action, data, context),

  performance: (label: string, startTime: number, context?: LogContext) =>
    logger.performance(label, startTime, context),

  form: (formName: string, success: boolean, data?: unknown, context?: LogContext) =>
    logger.formSubmission(formName, success, data, context),

  auth: (action: string, success: boolean, data?: unknown, context?: LogContext) =>
