...
export const requiredSelectSchema = z.string().min(1, 'Please select an option')
export const optionalSelectSchema = z.string().optional()

// Text content schemas
export const shortTextSchema = z.string().max(100, 'Text is too long (max 100 characters)')
export const mediumTextSchema = z.string().max(500, 'Text is too long (max 500 characters)')
export const longTextSchema = z.string().max(2000, 'Text is too long (max 2000 characters)')

// Arrays
export const nonEmptyArraySchema = <T>(itemSchema: z.ZodType<T>) =>
  z.array(itemSchema).min(1, 'At least one item is required')

export const optionalArraySchema = <T>(itemSchema: z.ZodType<T>) => z.array(itemSchema).optional()
