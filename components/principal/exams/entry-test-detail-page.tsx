...
import {
  useEntryTests,
  useEntryTestScoreSpectrum,
  useEntryTestStudentGrades
} from '@/hooks/api/principal/use-entry-tests'
...
import type { ScoreSpectrumStatistics } from '@/hooks/api/principal/use-entry-tests'
...
interface EntryTestDetailPageClientProps {
  entryTestId: string
}

export function EntryTestDetailPageClient({ entryTestId }: EntryTestDetailPageClientProps) {
  ...
  // Fetch entry tests list to find current test
  const allTestsQuery = useEntryTests({ limit: 100 })
  const currentTest = useMemo(
    () => allTestsQuery.data?.data?.entry_tests?.find((t) => t.entry_test_id === entryTestId),
    [allTestsQuery.data?.data?.entry_tests, entryTestId]
  )

  // Fetch score spectrum
  const scoreSpectrumQuery = useEntryTestScoreSpectrum(entryTestId)

  // Fetch student grades with debounced search
  const studentGradesQuery = useEntryTestStudentGrades(entryTestId, {
  ...
  const isLoading = allTestsQuery.isLoading || scoreSpectrumQuery.isLoading
  const hasError = allTestsQuery.isError || scoreSpectrumQuery.isError
        ...
        <div className='flex items-center gap-3 rounded-lg border border-destructive/50 bg-destructive/5 p-6 text-center'>
          ...
          <div className='text-left'>
            ...
            <p className='text-sm text-destructive/80'>
              {allTestsQuery.isError ? ErrorHandler.getErrorMessage(allTestsQuery.error) : 'Không tìm thấy bài thi'}
            </p>
          </div>
        </div>
        <Button onClick={() => allTestsQuery.refetch()} variant='outline' className='w-fit'>
          Thử lại
        </Button>
      </div>
    )
  ...
  return (
    <div className='flex flex-col gap-6 px-4 pb-10 pt-4 sm:px-6 lg:px-8'>
      ...
      <header className='space-y-2'>
        <div className='flex items-start justify-between gap-4'>
          <div className='flex-1 space-y-1'>
            <h1 className='text-2xl font-semibold text-greyscale-900'>{currentTest.title}</h1>
            <p className='text-sm text-greyscale-500'>{currentTest.description}</p>
          </div>
        ...
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4'>
          <div>
            ...
            <div className='mt-1 text-sm font-medium text-greyscale-900'>
              {format(new Date(currentTest.start_time), 'dd/MM/yyyy HH:mm')}
            </div>
          </div>
          <div>
            <div className='text-xs font-medium uppercase tracking-wide text-greyscale-600'>Kết thúc</div>
            <div className='mt-1 text-sm font-medium text-greyscale-900'>
              {format(new Date(currentTest.end_time), 'dd/MM/yyyy HH:mm')}
            </div>
          </div>
          <div>
            <div className='text-xs font-medium uppercase tracking-wide text-greyscale-600'>Người tạo</div>
            <div className='mt-1 text-sm font-medium text-greyscale-900'>{currentTest.created_by.name}</div>
          </div>
          <div>
            <div className='text-xs font-medium uppercase tracking-wide text-greyscale-600'>Số bộ câu hỏi</div>
            <div className='mt-1 text-sm font-medium text-greyscale-900'>{currentTest.question_sets.length}</div>
          </div>
        </div>
      </section>
      ...
      <ArrangeStudentsDrawer
        ...
        entryTestTitle={currentTest.title}
