...
export default function WebRTCRoom({ signalingUrl }: Props) {
  ...
  return (
    <div className='fixed inset-0 bg-[#202124] overflow-hidden'>
      ...
      {/* Header - Only show controls when not in meeting */}
      {!joined && (
        <div className='absolute top-4 left-4 right-4 flex gap-2 items-center z-30'>
          <input
            value={roomId}
            onChange={(e) => setRoomId(e.target.value)}
            placeholder='Room ID'
            className='flex-1 px-4 py-2 rounded-lg border border-gray-300 bg-white text-sm'
          />
          <input
            value={userId}
