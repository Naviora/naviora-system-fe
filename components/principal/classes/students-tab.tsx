...
import { useClassStudents } from '@/hooks/api/principal/use-classes'
import { createStudentColumns } from './student-columns'
import type { StudentRow, StudentDto } from '@/types/principal/classes'

interface StudentsTabProps {
  classId: string
}

function mapStudentsToRows(students: StudentDto[]): StudentRow[] {
...
export function StudentsTab({ classId }: StudentsTabProps) {
  ...
  const paginationData = studentsQuery.data?.data.pagination
  const mappedStudents = mapStudentsToRows(students)

  const columns = React.useMemo(() => createStudentColumns(), [])
